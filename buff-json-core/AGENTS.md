...
The core library. Contains the public API (`BuffJson`, `BuffJsonEncoder`) and all internal serialization logic.
...
io.suboptimal.buffjson/
  ...
  BuffJsonEncoder.java             # Immutable, thread-safe encoder with optional TypeRegistry + useGeneratedEncoders
...
io.suboptimal.buffjson.internal/
  ...
  ProtobufMessageWriter.java       # Main serialization — checks GeneratedEncoderRegistry, falls back to runtime
...
3. `ProtobufMessageWriter.writeMessage()` → `writeFields()`:
   - First checks `GeneratedEncoderRegistry.get(descriptor)`:
     ...
     - If found: delegates to `BuffJsonGeneratedEncoder.writeFields()` → **codegen path**
...
- `BuffJsonEncoder.encode()` sets `BuffJson.ACTIVE_REGISTRY` ThreadLocal before calling fastjson2

...
public final class GeneratedEncoderRegistry {
	...
	private static final ConcurrentHashMap<String, BuffJsonGeneratedEncoder<?>> ENCODERS_BY_NAME = new ConcurrentHashMap<>();
	private static final ConcurrentHashMap<Descriptor, BuffJsonGeneratedEncoder<?>> ENCODERS_BY_DESC = new ConcurrentHashMap<>();
	private static final boolean HAS_ENCODERS;

	static {
		ServiceLoader.load(BuffJsonGeneratedEncoder.class)
				.forEach(enc -> ENCODERS_BY_NAME.put(enc.descriptorFullName(), enc));
	...
	public static <T extends Message> BuffJsonGeneratedEncoder<T> get(Descriptor descriptor) {
		var cached = ENCODERS_BY_DESC.get(descriptor);
		if (cached != null) {
			return (BuffJsonGeneratedEncoder<T>) cached;
		}
		var encoder = ENCODERS_BY_NAME.get(descriptor.getFullName());
		if (encoder != null) {
			ENCODERS_BY_DESC.put(descriptor, encoder);
		}
		return (BuffJsonGeneratedEncoder<T>) encoder;
	}
}
